...
//Code Start
        if(GMint >= 2)
        {
         field_35366_j.displayString = "Game Mode: " + field_35364_f;
        field_35370_u = GMAPI.GMLine1[GMint];
        field_35369_v = GMAPI.GMLine2[GMint];
...
//Code Start
            ...
            {
               for( int ii = 1; 100 >= ii;ii++)
        {
              if(GMAPI.GMNameList.get(ii) != null && field_35364_f.equals(GMAPI.GMNameList.get(ii).toString()))
              {
                 i = ii;
                 mc.playerController = GMAPI.playerController[ii];
              }
        ...
        List GMNameList = GMAPI.GMNameList;
        List GMIDList = GMAPI.GMIDList;
        for( int ii = 2; ii <= 200 ;ii++)
        {
        if(GMNameList.get(ii) != null || GMIDList.get(ii) != null)
        {
        if(GMAPI.GMNameList.get(ii).toString() != LastGM)
        {
        if( ii > LastGMint){
        if(field_35364_f.equals(LastGM) || field_35364_f.equals("creative"))
        {
        field_35364_f = GMNameList.get(ii).toString();
        LastGM = GMNameList.get(ii).toString();
        LastGMint = ii;
        GMint = ii;
        ii = 201;
        }
...
//Code Stop
    ...
    private int GMint = 0;
    private int LastGMint = 0;
    private String LastGM = "";
